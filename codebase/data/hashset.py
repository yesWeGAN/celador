...
from codebase.data.dataset_structures import (
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Process dataset output into hashed dataset format."
    )
    parser.add_argument(
        "-d", "--dataset", type=str, default="celador", help="dataset specification"
    )
    parser.add_argument(
        "-t", "--transform", action="store_true", help="transform dataset from raw"
    )
...
class HashSetTransformer:
    """This class combines a dataset with images, image embeddings (EfficientNetB0), captions,
    caption embeddings (SentenceBERT) in one access structure.
    Used to transform existing datasets into a common format.

    Hashes image filenames to get unique identifiers."""

    def __init__(self, source_structure: dict):
        ...
        self._source_caption_data_dir = Path(
            os.path.join(self.source_structure["_source_caption_dir"], "captions")
        )
        self._source_caption_index_dir = Path(
            os.path.join(self.source_structure["_source_caption_dir"], "index")
        )
        self._source_embd_data_dir = Path(
            os.path.join(self.source_structure["_source_embd_dir"], "embd")
        )
        self._source_embd_index_dir = Path(
            os.path.join(self.source_structure["_source_embd_dir"], "index")
        )

        # set up the target dir
        self.basepath = Path(
            os.path.join(
                self.source_structure["target_dir"],
                self.source_structure["dataset_name"],
            )
        )
    ...
    def hash_images(self, dirname, hashname):
        ...
        for file in allfiles:
            try:
                ...
                future_imgfile = os.path.join(
                    self._target_img_subdir, hashname, f"{hash_filename}{file.suffix}"
                )
                ...
                if hash_filename in self.samples.keys():
                    ...
                    self.samples[hash_filename] = Sample(
                        hashid=hash_filename,
                        parent=dirname,
    ...
    def hash_caption_json_files(self, dirname, hashname):
        try:
            ...
            raise FileNotFoundError(
                f"No caption json file found for {dirname}. Exiting."
            )

        future_cap_jsonfile = os.path.join(
            self._target_cap_knn_tensor_subdir, f"{hashname}_caption_tensors.pt"
        )
        ...
        if self.copy:
            json.dump(
                ...
                open(
                    os.path.join(
                        self._target_json_backup_dir, f"{hashname}_captions.json"
                    ),
                    "w",
                ),
            )
    ...
    def hash_embd_json_files(self, dirname, hashname):
        ...
        future_embd_jsonfile = os.path.join(
            self._target_embd_knn_tensor_subdir, f"{hashname}_embd_tensors.pt"
        )
        jsondata = json.load(open(filep, "r"))
            ...
            try:
                ...
                if "instagram" in self.sourcepath.as_posix():
                    imgpath = os.path.join(
                        self._source_img_dir, dirname, Path(imgpath).name
                    )
        ...
        if self.copy:
            json.dump(
                ...
                open(
                    os.path.join(
                        self._target_json_backup_dir, f"{hashname}_embd_indices.json"
                    ),
                    "w",
                ),
            )
    ...
    def hash_caption_tensors(self, dirname, hashname):
        prev_cap_tensorfile = os.path.join(
            self._source_caption_data_dir, dirname, "tensors.pt"
        )
        future_cap_tensorfile = os.path.join(
            self._target_cap_knn_tensor_subdir, f"{hashname}_caption_tensors.pt"
        )
        if self.copy:
    ...
    def hash_embd_tensors(self, dirname, hashname):
        prev_embd_tensorfile = os.path.join(
            self._source_embd_data_dir, dirname, "tensors.pt"
        )
        future_embd_tensorfile = os.path.join(
            self._target_embd_knn_tensor_subdir, f"{hashname}_embd_tensors.pt"
        )
        if self.copy:
        ...
        for attr, state in vars(self).items():
            if isinstance(attr, Path):
                ...
                if len(state) == 0:
                    ...
                    keyset = [
                        key.as_posix() if isinstance(key, Path) else key
    ...
    def save(self):
        ...
        with open(
            os.path.join(self._target_dataset_dir, f"hashset_{date_time}.json"), "w"
        ) as jsonout:
...
class HashSet(torch.utils.data.Dataset):
    def __init__(self, datapath=None):
        try:
            ...
            raise FileNotFoundError(
                f"No dataset json file found in this directory. Exiting."
            )
    ...
    def process_result_list(self, resultlist: list):
        ...
        for k, result in enumerate(resultlist):
            resultlist[k].result_samples = [
                self.samples[self.idx_to_hash[idx]] for idx in result.result_indices
            ]
        return resultlist
    ...
    def check_sample_completeness(self):
        ...
        for key, sample in self.samples.items():
            if (
                sample.embd_tensor_index is not None
                and sample.cap_tensor_index is not None
            ):
    ...
    def load_tensors(self):
        ...
        current_embd_tensor = torch.load(
            open(current_sample.embd_tensor_path, "rb")
        ).cpu()
        current_cap_tensor = torch.load(
            open(current_sample.cap_tensor_path, "rb")
        ).cpu()

        for k, (hashid, sample) in enumerate(self.samples.items()):
            if k != len(embd_tensors):
                print(
                    f"Diverging index and tensor length: k {k} and {len(embd_tensors)}"
                )
                sys.exit(1)

            if sample.embd_tensor_path == current_embd_tensorpath:
                if (
                    len(current_embd_tensor.shape) < 2
                    or len(current_cap_tensor.shape) < 2
                ):
                    ...
                    embd_tensors.append(
                        current_embd_tensor[int(sample.embd_tensor_index)].squeeze()
                    )
                    cap_tensors.append(
                        current_cap_tensor[int(sample.cap_tensor_index)].squeeze()
                    )
                    self.hash_to_index_idx[sample.id] = k
                    self.samples[sample.id].knn_idx = k
            else:
                ...
                current_embd_tensor = torch.load(
                    open(sample.embd_tensor_path, "rb")
                ).cpu()
                current_cap_tensor = torch.load(
                    open(sample.cap_tensor_path, "rb")
                ).cpu()
                if (
                    len(current_embd_tensor.shape) < 2
                    or len(current_cap_tensor.shape) < 2
        ...
        assert len(cap_tensors) == len(
            embd_tensors
        ), "embd and caption tensors are not of same lenght!"
        ...
        torch.save(
            embd_cat,
            os.path.join(
                self._target_embd_knn_tensor_subdir, "embd_vectors_combined.pt"
            ),
        )
        gc.collect()
        ...
        torch.save(
            cap_cat,
            os.path.join(
                self._target_cap_knn_tensor_subdir, "caption_vectors_combined.pt"
            ),
        )
        gc.collect()
        ...
        torch.save(
            combi_cat,
            os.path.join(self._target_combi_knn_tensor_subdir, "combivector.pt"),
        )

        assert int(combi_cat.shape[1]) == int(
            embd_cat.shape[1] + cap_cat.shape[1]
        ), f"Img/cap tensors concatenated in wrong dimension. Shape: {combi_cat.shape}"
