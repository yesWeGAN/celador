...
class SampleVisualizer:
    ...
    def __init__(
        ...
        self.default_output_dir = (
            ...
            else "/home/frank/data/apps" "/default_plot_output"
        )
    ...
    def create_grid(self, samplelist: list[Sample]):
        ...
        num_rows = (
            len(samplelist) // self.max_cols
            if len(samplelist) % self.max_cols == 0
            else (len(samplelist) // self.max_cols + 1)
        )
    ...
    def create_knn_result_grid(self, result, hashset: HashSet):
        ...
        num_rows = (
            len(result.result_samples) // self.max_cols
            if len(result.result_samples) % self.max_cols == 0
            else (len(result.result_samples) // self.max_cols + 1)
        )
        gs = gridspec.GridSpec(num_rows + 1, self.max_cols)
        fig = plt.figure(figsize=self.plot_size)
        fig.tight_layout()
        sample_result_representations = [
            sample.vis_representation() for sample in result.result_samples
        ]
        ...
        ax.set_title(
            f"Your query: {query_representation['id']} \n{query_representation['textboxstr']}",
            fontsize=8,
        )
